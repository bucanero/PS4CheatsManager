    ...
    while ((option_name = menu_options[ind].name))
    {
		...
		switch (menu_options[ind].type)
		{
			...
			case APP_OPTION_CALL:
				DrawTexture(orbisPadGetConf()->crossButtonOK ? &menu_textures[footer_ico_cross_png_index] : &menu_textures[footer_ico_circle_png_index], OPTION_ITEM_OFF - 29, y_off+2, 0, menu_textures[footer_ico_cross_png_index].width/2, menu_textures[footer_ico_cross_png_index].height/2, 0xFFFFFF00 | alpha);
				break;
...
void Draw_OptionsMenu_Ani()
{
    ...
    for (ani = 0; ani < MENU_ANI_MAX; ani++)
    {
        ...
        DrawHeader_Ani(header_ico_opt_png_index, "Settings", NULL, APP_FONT_TITLE_COLOR, 0xffffffff, ani, 12);
