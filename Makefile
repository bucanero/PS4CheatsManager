...
eboot.bin: $(INTDIR) $(OBJS)
	...
	$(TOOLCHAIN)/bin/$(CDIR)/create-fself -in=$(INTDIR)/$(PROJDIR).elf -out=$(INTDIR)/$(PROJDIR).oelf --eboot "$(MAIN_SELF)" --paid 0x3800000000000011 --authinfo 
...
appzip: $(CONTENT_ID).pkg
	@zip -r goldcheats.zip assets $(MAIN_SELF)
